...
| `find_profile_by_handle(p_username, p_code)` | you can't see a stranger's profile at all. Needs the username (case-insensitive) **and** the permanent 6-character friend code (0019; Crockford base32 since 0020, case-insensitive, O/I/L read as 0/1); returns `id`, `display_name`, `avatar_hue`, `avatar_url`. Replaces `find_profile_by_email` |
